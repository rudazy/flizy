...
const { CHANNELS, normalizeChannel, normalizeExternalId } = require('./channelKey');
...
async function consumeLinkCode(channel, externalId, codeRaw, verifiedPhone) {
  ...
  // Only a code that does not exist counts as a guess. An expired code is one
  // the account holder really was issued, so a slow legitimate user is never
  // pushed up the ladder, and it costs nothing: at 50 bits, a brute forcer's
  // guesses land on 'invalid' essentially every time.
  if (!row) {
