 ...
 * @param {{ displayHandle?: string|null }} [opts] optional display handle (e.g. Telegram @username)
 * @returns {Promise<{ ok: boolean, reason?: string, account?: object, identity?: object, rebound?: boolean, phone?: string|null }>}
 */
async function consumeLinkCode(channel, externalId, codeRaw, verifiedPhone, opts = {}) {
  const supabase = getSupabase();
  ...
  try {
    await bindChannelIdentity(supabase, {
      ...
      // Display only. Claims and future routing match external_id, not the handle.
      displayHandle: opts.displayHandle || null,
...
/**
 * Soft-refresh Telegram display_handle when the user messages the bot.
 * Enables pay-by-@username for people already linked (Bot API cannot resolve private users).
 * Never throws; never invents a handle.
 *
 * @param {string} externalId Telegram user id
 * @param {string|null|undefined} username bare @username without @
 */
async function refreshTelegramDisplayHandle(externalId, username) {
  const id = normalizeExternalId(externalId);
  ...
  try {
    ...
    const { data: existing, error: readErr } = await supabase
      ...
      .eq('channel', CHANNELS.TELEGRAM)
      .eq('external_id', id)
      .maybeSingle();
    if (readErr || !existing) return;
    if (String(existing.display_handle || '') === handle) return;
    ...
    const { error: upErr } = await supabase
      ...
      .eq('id', existing.id);
    if (upErr) {
      console.warn('[identity] refresh telegram handle:', upErr.message);
    }
  } catch (err) {
    console.warn('[identity] refresh telegram handle:', err && err.message ? err.message : err);
  }
}
