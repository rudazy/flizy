...
/**
 * Channels where an account may hold at most one identity.
 *
 * Chat channels are deliberately excluded. A WhatsApp LID changes when somebody
 * swaps device, and the way back in is to redeem a link code, which inserts a
 * second whatsapp row on the same account. Holding chat channels to one-per-
 * account would break that recovery path. The database says the same thing:
 * channel_identities_account_platform_idx is partial over exactly this set.
 */
const PLATFORM_CHANNELS = Object.freeze(new Set([CHANNELS.X, CHANNELS.GITHUB, CHANNELS.DISCORD]));

/** @param {string} raw */
function isPlatformChannel(raw) {
