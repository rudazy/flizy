 ...
 *
 * Registration email (accounts.email) only when email_verified_at is set.
 * Secondary rows only when account_emails.verified_at is set.
 * Unverified addresses must never receive holds.
 */
...
async function listClaimableEmailsForAccount(accountId) {
  ...
  // Ownership proof required — unverified registration email is not claimable.
...
async function listAccountEmails(accountId) {
  ...
  if (eErr) {
    if (String(eErr.message || '').includes('account_emails') || eErr.code === '42P01') {
      return { primary, primaryVerified, additional: [] };
    }
    throw new Error(eErr.message);
  }
