...
const config = {
  ...
  /**
   * How often a client process drains notifications queued for its channel.
   * Only reached when the notifying process cannot deliver on that channel
   * itself (a Telegram-originated claim addressed to a WhatsApp user), so this
   * is the worst case for "notified immediately", not the normal case.
   */
  outboxDrainMs: envNumber('OUTBOX_DRAIN_MS', 5000),
  ...
  siteUrl: envString('SITE_URL', 'https://flizy.app').replace(/\/$/, ''),
