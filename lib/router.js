...
function helpText(ctx) {
  ...
  const lines = [
    ...
    p('send 0.01 to @rudazy on github'),
    '  confirm',
    '  cancel',
    '',
    ...
    '',
    'Claims (phone or GitHub not linked yet):',
    `${p('send 0.01 to 234…')}  → hold until they link Flizy`,
    `${p('send 0.01 to @user on github')}  → hold for that GitHub`,
    `${p('cancel claims')}       → cancel anytime (1/2/3/All)`,
    `${p('claim')}               → receive claims for your phone or linked GitHub`,
    '',
...
function commandMenu() {
  return [
    ...
    { command: 'send', description: 'Send: /send 0.01 to @user on github' },
    { command: 'request', description: 'Ask for money: /request 0.01 from 234…' },
    { command: 'pay', description: 'Pay requests addressed to you' },
    { command: 'claim', description: 'Receive claims for phone or linked GitHub' },
    ...
    try {
      ...
      await notifyAccount(
        ...
        formatClaimClaimedNotice({
          ...
          byLabel,
          forLabel: claimRecipientLabel(result.claim),
          explorerUrl: result.explorerUrl || null,
        }),
        { skip: [{ channel: ctx.channel, externalId: ctx.externalId }] }
      );
