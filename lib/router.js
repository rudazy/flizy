...
const { platformRecipient, emailRecipient } = require('./claimRecipient');
...
const SEND_PLATFORM_RE = 'github|discord|x|twitter|telegram|tg';
...
function emailClaimLinkHowTo() {
  return 'They receive after they create a Flizy account with that email (or add and verify it), then claim on the site or in chat.';
}
...
async function handleSendPlatformClaim(ctx, user, siteAcc, amountEth, handleRaw, platform) {
  ...
  try {
    if (ch === CHANNELS.GITHUB) {
      ...
      if (!profile) {
        await reply(ctx, telegramNotFoundMessage((body) => cmd(ctx, body)));
        return;
      }
      // Prefer a normalized public handle when the API returned one.
      if (profile.login) {
