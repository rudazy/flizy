...
const { resolveDiscordUser } = require('./discordLookup');
    ...
    if (ch === CHANNELS.GITHUB) {
      ...
      if (!profile) {
        await reply(
          ctx,
          [
            'Could not find that Discord user on Flizy by name.',
            'Use their Discord user id (Developer Mode → right-click user → Copy User ID).',
            `Example: ${cmd(ctx, 'send 0.001 to 123456789012345678 on discord')}`,
            'Or they can link Discord on Flizy first, then you can use their handle.',
          ].join('\n')
        );
        return;
      }
