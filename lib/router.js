...
function normalizePlatformName(raw) {
  ...
  if (p === 'github' || p === 'discord' || p === 'x') return p;
  return null;
}
...
function parseSendCommand(text) {
  ...
  // Platform before bare alias: "to @user on github|discord|x"
  const assetPlatformOn = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s+([a-zA-Z][a-zA-Z0-9]{0,9})\s+to\s+@?([a-zA-Z0-9_.-]{1,39})\s+on\s+(github|discord|x|twitter)\b/i
  );
  if (assetPlatformOn) {
    return {
  ...
  const assetPlatformColon = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s+([a-zA-Z][a-zA-Z0-9]{0,9})\s+to\s+(github|discord|x|twitter):([a-zA-Z0-9_.-]{1,39})\b/i
  );
  ...
  // Preferred UX: send 0.001 to @user on github|discord|x
  const platformOn = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s*(?:eth)?\s+to\s+@?([a-zA-Z0-9_.-]{1,39})\s+on\s+(github|discord|x|twitter)\b/i
  );
  if (platformOn) {
    return {
  ...
  // Shorthand: send 0.001 to github:user | discord:id | x:handle
  const platformColon = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s*(?:eth)?\s+to\s+(github|discord|x|twitter):([a-zA-Z0-9_.-]{1,39})\b/i
  );
  if (platformColon) {
    return {
...
async function handleSend(
  ...
  // --- Platform: claim hold by immutable id (github / discord / x) ---
  if (platform === 'github' || platform === 'discord' || platform === 'x') {
...
/**
 * Hold ETH for a platform identity (GitHub / Discord / X).
...
async function handleSendPlatformClaim(ctx, user, siteAcc, amountEth, handleRaw, platform) {
  ...
  const where =
    ch === CHANNELS.GITHUB ? 'GitHub' : ch === CHANNELS.DISCORD ? 'Discord' : 'X';
