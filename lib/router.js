...
async function handleConfirm(ctx, user, account) {
  ...
  if (plan.intent === 'CLAIM_HOLD') {
    ...
    const notified = await notifyClaimTarget(
      ctx,
      toWaHint,
      plan.input.amount,
  ...
  if (paymentRequestId) {
    let held = null;
    try {
      held = await beginRequestProcessing(paymentRequestId);
    } catch (err) {
...
/**
 * How to name the sender to somebody who is about to be told they got money.
 * The verified phone first (that is what a recipient recognises), then the
 * display name. Never the email: a notification is not a place to leak one.
 *
 * @param {object} ctx
 * @param {string|null} accountId sender account
 * @returns {Promise<string>}
 */
async function senderLabel(ctx, accountId, identity) {
  try {
