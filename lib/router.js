 ...
 *
 * Prefer display name (and later Flizy @username) so senders are not confused
 * by raw phones. Phone is a fallback when no name is set. Never email.
 *
 * @param {object} ctx
 * @param {string|null} accountId
...
async function senderLabel(ctx, accountId, identity, opts = {}) {
  ...
  if (preferPhone) {
    return (await phoneLabel()) || (await nameLabel()) || 'another Flizy user';
  }
  // Claimed-by and similar: name first (username later plugs in here)
  return (await nameLabel()) || (await phoneLabel()) || 'another Flizy user';
}
