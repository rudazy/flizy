 ...
 *
 * @type {Map<string, {
 *   options: Array<{ keyword: string, label: string, command: string }>,
 *   createdAt: number,
 * }>}
 */
...
function parseSendCommand(text) {
  ...
  const payCode = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s*(?:eth)?\s+to\s+([2-9A-HJ-NP-Za-hj-np-z]{6})\s*$/i
  );
...
function isFlizyCommand(ctx, text) {
  ...
  if (
    pendingMerchantSaves.has(ctx.key) &&
    (low === 'save' ||
      low === 'yes' ||
      low === 'skip' ||
      low === 'no' ||
...
function normalizeInput(ctx, rawText) {
  ...
  const midFlow = flow.walletAdd || flow.claimMenu || flow.unlock || flow.choice;
...
function helpText(ctx) {
  ...
  return [
    ...
    `  ${p('send 0.01 to friend@email.com')}   hold for an email`,
    `  ${p('send 0.01 to @user on telegram')}   hold for a Telegram user`,
...
function welcomeText(ctx, user) {
  return [
    'Welcome to Flizy',
    '',
    'Send money like a message — to people you trust, a phone, email, Telegram, or GitHub.',
    '',
    isTelegram(ctx)
...
function commandMenu() {
  return [
    ...
    { command: 'unlink', description: 'Disconnect this chat from Flizy' },
    ...
    { command: 'send', description: 'Pay a name, phone, email, Telegram, or GitHub' },
    ...
    { command: 'buy', description: 'Optional: buy FLZ (buy 100 FLZ)' },
    { command: 'sell', description: 'Optional: sell FLZ' },
    { command: 'swap', description: 'Optional: swap tokens' },
    { command: 'trade', description: 'Optional: trade FLZ, asks which way' },
...
async function handleDeposit(ctx, user, account) {
  ...
  const lines = [
    ...
    'Fund with the official GIWA faucet:',
    '  1) Copy your Flizy wallet address (below or on the site)',
    '  2) Open https://faucet.giwa.io',
    '  3) Paste your Flizy address and request test ETH',
    `Dashboard: ${config.siteUrl}/dashboard/wallet?s=fund`,
    '',
  ];
  try {
    const acc = await resolveLinkedSiteAccount(ctx, account);
...
async function handleSend(
  ...
  // Token claims to unlinked phones/platforms/emails are not supported (escrow is ETH only)
  if ((isPhone || isPlatform || isEmail) && !native) {
    await reply(
      ctx,
      [
  ...
  if (
    ...
    await handleSendPlatformClaim(ctx, user, siteAcc, amountEth, toRaw, platform);
    return;
  }

  // --- Email: claim hold for registration or verified secondary email ---
  if (isEmail) {
    await handleSendEmailClaim(ctx, user, siteAcc, amountEth, toRaw);
    return;
  }
  ...
  if (!isAddress) {
    ...
    if (dest.found && dest.address) {
      ...
      return handleSendResolved(
        ...
        siteAcc,
        { skipTrusted: true, asset: sendAsset, note, firstPay, offerSave }
      );
    }
  }
...
/**
 * Hold ETH for an email address (registration email or verified secondary).
 */
async function handleSendEmailClaim(ctx, user, siteAcc, amountEth, emailRaw) {
  let recipient;
  try {
...
/**
 * Hold ETH for a platform identity (GitHub / Discord / X / Telegram).
 * Handle is resolved to immutable id at plan time; money routes on the id only.
 */
async function handleSendPlatformClaim(ctx, user, siteAcc, amountEth, handleRaw, platform) {
  const ch =
    platform === 'github'
      ? CHANNELS.GITHUB
      : platform === 'discord'
  ...
  const intent = createSendIntent({
    ...
    toAddress: null,
    toLabel: claimRecipientLabel(recipient),
    // Prefer handle for logs; fall back to id when we only have a snowflake
 ...
 * @returns {Promise<boolean>}
 */
async function notifyClaimEmailTarget(ctx, email, amountEth, fromAccountId, identity) {
  try {
...
async function handleChoiceReply(ctx, user, account, text) {
  ...
  if (/^\d+$/.test(t)) {
    ...
    picked = choice.options[idx];
  } else {
    picked = choice.options.find((o) => o.keyword === t || o.command.toLowerCase() === t) || null;
  }
...
async function handle(ctx, rawText) {
  try {
    ...
    if (pendingPayAsks.has(ctx.key)) {
      ...
      if (!parseSendCommand(text) && !parseSwapCommand(text) && !parsePayAskCommand(text)) {
        await reply(
          ctx,
          'Send their @username or the 6-character code under the QR.\nOr: cancel'
        );
        return;
      }
      pendingPayAsks.delete(ctx.key);
