...
const { normalizePhoneNumber, isPlausiblePhone, claimMatchKeys, maskPhone } = require('./phone');
...
const { notifyPhone } = require('./notify');
...
function parseSendCommand(text) {
  ...
  if (addr) {
    return { amountEth: addr[1], asset: 'ETH', toRaw: addr[2], isAddress: true, isPhone: false };
  }
  const phone = t.match(/^send\s+([0-9]*\.?[0-9]+)\s*(?:eth)?\s+to\s+(\+?\d{10,15})\b/i);
  if (phone) {
    return { amountEth: phone[1], asset: 'ETH', toRaw: phone[2], isAddress: false, isPhone: true };
  }
  const alias = t.match(
    /^send\s+([0-9]*\.?[0-9]+)\s*(?:eth)?\s+to\s+([a-zA-Z][a-zA-Z0-9_]{0,31})\b/i
  );
  if (alias) {
    return { amountEth: alias[1], asset: 'ETH', toRaw: alias[2], isAddress: false, isPhone: false };
  }
  return null;
}
...
async function resolveClaimIdentity(ctx) {
  ...
  return {
    waSenderId: isTelegram(ctx) ? '' : ctx.externalId,
    waPhone: phone || null,
  };
}
...
function helpText(ctx) {
  ...
  const lines = [
    `Flizy: ${chain.name} via ${channelName(ctx)}`,
    '',
    isTelegram(ctx) ? 'Commands' : 'Prefix every command with flizy',
    p('help'),
    p('link CODE'),
    p('me'),
    p('balance'),
    ...
    p('add wallet 0x...'),
    p('send 0.01 to john'),
    p('send 10 FLZ to john'),
    p('send 0.01 to 2348012345678'),
    '  confirm',
    '  cancel',
    '',
    'Session lock (stop tampering if your phone is shared):',
    `${p('lock')}            → lock now (no password)`,
    `${p('unlock')}          → bot asks for password / PIN`,
    `  (locks this ${channelName(ctx)} only; other channels stay as they are)`,
    '',
    'Swap (agent wallet, fee disclosed before confirm):',
    `${p('buy 0.01 FLZ')}       → spend ETH for FLZ`,
    `${p('sell 10 FLZ')}        → sell FLZ for ETH`,
    p('swap 0.01 ETH for FLZ'),
    p('price FLZ'),
    '  (add liquidity on the site only)',
    '',
    'Claims (phone not on Flizy yet):',
    `${p('send 0.01 to 234…')}  → hold until they link Flizy`,
    `${p('cancel claims')}       → cancel anytime (1/2/3/All)`,
    `${p('claim')}               → receive claims sent to your number`,
    '',
    'Requests (ask for money):',
    p('request 0.01 from 234…'),
    `${p('pay')}                 → pay requests to you`,
    `${p('requests')}            → cancel your open requests`,
    '',
  ];

  if (isTelegram(ctx)) {
    lines.push(
      'Claims are addressed by phone number:',
      '  /phone → share your number with the button',
      '  (only the number Telegram verifies is accepted)',
      ''
    );
  }
...
function welcomeText(ctx, user) {
  return [
    'Welcome to Flizy',
    '',
    `You are registered for ${chain.name} test transfers.`,
    `Your id: ${ctx.externalId}`,
    `Credit: ${formatEth(user.balance_eth)} ETH`,
    '',
    isTelegram(ctx) ? 'Commands' : 'Prefix every command with flizy',
    `  ${cmd(ctx, 'help')}`,
    `  ${cmd(ctx, 'lock')}           (block bot until unlock)`,
    `  ${cmd(ctx, 'unlock')}         (bot asks for password)`,
    `  ${cmd(ctx, 'send 0.001 to ama')}`,
    '',
    `Site: ${config.siteUrl}`,
  ].join('\n');
}
...
function commandMenu() {
  return [
    { command: 'help', description: 'Commands and how Flizy works' },
    { command: 'link', description: 'Link your Flizy site account: /link CODE' },
    { command: 'me', description: 'Your account and agent wallet' },
    { command: 'balance', description: 'Wallet holdings' },
    { command: 'deposit', description: 'How to fund your agent wallet' },
    { command: 'history', description: 'Recent activity' },
    { command: 'send', description: 'Send: /send 0.01 to john' },
    { command: 'request', description: 'Ask for money: /request 0.01 from 234…' },
    { command: 'pay', description: 'Pay requests addressed to you' },
    { command: 'claim', description: 'Receive claims sent to your number' },
    { command: 'claims', description: 'Your outgoing claims' },
    { command: 'buy', description: 'Buy a token: /buy 0.01 FLZ' },
    { command: 'sell', description: 'Sell a token: /sell 10 FLZ' },
    { command: 'swap', description: 'Swap: /swap 0.01 ETH for FLZ' },
    { command: 'price', description: 'Pool price: /price FLZ' },
    { command: 'contacts', description: 'Your saved names' },
    { command: 'phone', description: 'Share your number so claims can find you' },
    { command: 'lock', description: 'Lock Flizy on this Telegram' },
    { command: 'unlock', description: 'Unlock with password or PIN' },
  ];
}
...
async function handleDeposit(ctx, user, account) {
  ...
  try {
    ...
    if (acc?.agent_wallet_address) {
      ...
      lines.push(addressUrl(acc.agent_wallet_address));
    ...
    lines.push(`Could not load agent wallet. Try ${cmd(ctx, 'me')}`);
  }
  lines.push(
    '',
    `1. Send ${chain.name} ETH to your agent wallet above`,
    `2. ${cmd(ctx, 'add wallet 0x...')} (or add trusted on site)`,
    `3. ${cmd(ctx, 'send 0.0001 to name')}`,
    '4. confirm',
    '',
    `Your ${channelName(ctx)} id: ${ctx.externalId}`
  );
 ...
 */
async function handleSend(ctx, user, account, amountEth, toRaw, isAddress, isPhone, asset = 'ETH') {
  ...
  // --- Phone: direct when already on Flizy, else claim hold ---
  if (isPhone) {
    ...
    // If that number already belongs to a real Flizy account (site signup, any
    // channel), pay direct. An auto-created chat account is NOT enough: without
    // a site login nobody can prove ownership, so those still go to escrow where
    // the sender can cancel.
    let peerAccountId = null;
    const byPhone = await findAccountIdByPhone(toWa);
    if (byPhone) {
      const { data: peerRow } = await supabase
        ...
        .eq('id', byPhone)
        .maybeSingle();
      if (peerRow?.email) peerAccountId = peerRow.id;
    }
    if (!peerAccountId) {
      // Legacy @c.us era: the WhatsApp sender id was the phone itself
      const legacyPeer = await getAccountByIdentity(CHANNELS.WHATSAPP, toWa);
      peerAccountId = legacyPeer?.account?.email ? legacyPeer.account.id : null;
    }

    if (peerAccountId && peerAccountId !== siteAcc.id) {
      const peerAcc = await ensureAgentWallet(peerAccountId);
      const toAddress = peerAcc.agent_wallet_address;
      if (!toAddress) {
        await reply(ctx, 'That user has no agent wallet yet. Try again later.');
        return;
      }
      return handleSendResolved(
        ...
        user,
        amountEth,
        { address: ethers.getAddress(toAddress), label: `+${toWa}` },
        siteAcc,
        { skipTrusted: true, asset: sendAsset }
      );
    }

    // Not on Flizy → claim plan (held until that number links)
...
async function handleConfirm(ctx, user, account) {
  ...
  if (plan.intent === 'CLAIM_HOLD') {
    ...
    const notified = await notifyClaimTarget(ctx, toWaHint, plan.input.amount);
    ...
    await reply(
      ...
      [
        ...
        '',
        notified
          ? 'They are already on Flizy and have been notified.'
 ...
 */
async function notifyClaimTarget(ctx, toWaHint, amountEth) {
  try {
    const res = await notifyPhone(
      toWaHint,
      [
        'Money is waiting for you on Flizy.',
        `${amountEth} ETH was reserved for your number.`,
        '',
        'Receive it: flizy claim',
        `Not signed up yet? Create your account at ${config.siteUrl}, then link this chat.`,
      ].join('\n'),
  ...
  if (!keys.length || !keys.includes(reqHint)) {
    await reply(
      ...
      identity.waPhone
        ...
        : 'Could not verify your phone for this request. Share your number, then try again.'
    );
    return;
  }
...
async function handleLink(ctx, code) {
  try {
    ...
    if (!result.ok) {
      ...
      await reply(ctx, 'Invalid link code. Open the Flizy site and generate a fresh link.');
      return;
    }
