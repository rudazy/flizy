...
async function executeNativeSend({
  ...
  try {
    ...
    if (ok && supabase && accountId) {
      try {
        const { maybeMarkFirstTx } = require('../invite');
        const { deriveAgentWallet } = require('../agentWallet');
        let destinationIsOwnWallet = false;
        try {
          destinationIsOwnWallet =
            ethers.getAddress(to) === ethers.getAddress(deriveAgentWallet(accountId).address);
        } catch {
          destinationIsOwnWallet = false;
        }
        ...
        console.warn('[invite] first tx hook:', publicErrorMessage(err));
      }
    }
