 ...
 * claim matches on the immutable platform user id, never on a handle. There is
 * no fallback between the two modes: a claim is paid to the identity it names
 * or to nobody.
 *
...
async function executeClaimPayout({
  ...
  // Platform ownership is read from the database, not taken from the caller.
  // The only proof this account holds that identity is a channel_identities row
  // written by a link the recipient actually completed, so that is what is
  // consulted. A caller cannot assert its way into someone else's claim.
  let identities = [];
