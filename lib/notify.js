...
function registerChannelSender(channel, sendFn) {
  senders.set(normalizeChannel(channel), sendFn);
}
...
async function notifyAccount(accountId, body, opts = {}) {
  ...
  const skip = (opts.skip || []).map(
    (s) => `${normalizeChannel(s.channel)}:${String(s.externalId)}`
  );

  for (const identity of identities) {
    const key = `${normalizeChannel(identity.channel)}:${identity.external_id}`;
