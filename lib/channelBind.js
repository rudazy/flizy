...
/**
 * Remove a platform identity from an account.
 *
 * Callers gate this on a password re-entry: unlinking changes where future
 * payments can go, so it is held to the same bar as editing the trusted list.
 * Chat channels are refused here, because a whatsapp or telegram row carries
 * phone_e164, which is the join key for phone claims.
 */
