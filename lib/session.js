 ...
 *
 * The ladder itself is in lib/lockoutLadder.js because link codes escalate on
 * exactly the same steps. It can be this steep only because there is a way back
 * in that does not involve waiting: proving the account password on the site
 * clears it (web/app/api/pin/route.ts). Keep the two in step.
 */
const PIN_FREE_ATTEMPTS = FREE_ATTEMPTS;
const PIN_LOCKOUT_LADDER_MS = LOCKOUT_LADDER_MS;
const pinLockoutMsForAttempts = lockoutMsForAttempts;
...
function pinLockState(session) {
  return lockStateFrom(session?.pin_locked_until);
}
...
async function recordFailedPinAttempt(accountId, channel, externalId, session) {
  const ch = assertChannel(channel, 'recordFailedPinAttempt');
...
async function clearPinLockout(accountId, channel, externalId) {
  ...
  const { error } = await supabase
    ...
    .eq('account_id', accountId)
    .eq('channel', assertChannel(channel, 'clearPinLockout'))
...
async function getSession(accountId, channel, externalId) {
  ...
  const { data, error } = await supabase
    ...
    .select('*')
    .eq('account_id', accountId)
    .eq('channel', assertChannel(channel, 'getSession'))
    .eq('external_id', normalizeExternalId(externalId))
    .maybeSingle();
...
async function touchSession(accountId, channel, externalId) {
  ...
  const { error } = await supabase.from('sessions').upsert(
    {
      account_id: accountId,
      channel: assertChannel(channel, 'touchSession'),
...
async function unlockWithPin(account, channel, externalId, secret) {
  ...
  // Above the session read on purpose. The read below swallows its error, so an
  // unknown channel would otherwise land on session = null, which pinLockState
  // reads as "no lock" and would hand a guesser an uncounted attempt.
  const ch = assertChannel(channel, 'unlockWithPin');
  ...
  let session = null;
  try {
    session = await getSession(account.id, ch, externalId);
  } catch (err) {
  ...
  if (lock.locked) {
    console.warn(
      `[unlock] refused, locked out account=${account.id} channel=${ch} until=${lock.until}`
    );
    return {
      ok: false,
      reason: 'pin_locked',
  ...
  if (!ok) {
    ...
    const counted = await recordFailedPinAttempt(account.id, ch, externalId, session);
    console.warn(
      `[unlock] bad secret account=${account.id} channel=${ch} hasPin=${hasPin} hasPassword=${hasPassword} len=${secretTrimmed.length} attempts=${counted.attempts}`
    );
    return {
      ok: false,
      reason: 'bad_pin',
  ...
  await touchSession(account.id, ch, externalId);
  // A correct secret is one of only two things that may clear the counter.
  await clearPinLockout(account.id, ch, externalId);
  return { ok: true };
}
...
async function lockSession(accountId, channel, externalId) {
  ...
  const ch = assertChannel(channel, 'lockSession');
  const id = normalizeExternalId(externalId);
