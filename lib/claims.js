 ...
 *   email     to_email, proven by owning that address on the Flizy account
 *
 * Phone claims match via identity.phone_e164, never by LID alone. Platform
 * claims match on the immutable user id, never on the handle. Email claims
 * match the registration email and verified secondary emails. The rules live in
...
async function createClaim(p) {
  ...
  // Attribution only. Missing table/column or a failed lookup must not block the hold.
  let inviteCode = null;
  try {
    const { inviteCodeIfAttachEnabled } = require('./invite');
    inviteCode = await inviteCodeIfAttachEnabled(supabase, p.fromAccountId);
  } catch (err) {
    console.warn('[claim] invite attach lookup:', err && err.message ? err.message : err);
  }
  if (inviteCode) row.invite_code = inviteCode;

  let { data, error } = await supabase.from('claims').insert(row).select('*').single();
  if (error && inviteCode && /invite_code/i.test(String(error.message || ''))) {
    delete row.invite_code;
    inviteCode = null;
    const retry = await supabase.from('claims').insert(row).select('*').single();
    data = retry.data;
    error = retry.error;
  }
  if (error) throw new Error(error.message);

  const claimUrl = inviteCode
    ? `${config.siteUrl}/claim/${claim_token}?i=${inviteCode}`
    : `${config.siteUrl}/claim/${claim_token}`;
...
async function listIncomingPending(identityOrSender) {
  ...
  if (!keys.phones.length && !keys.identities.length && !keys.emails.length) return [];
  ...
  if (keys.emails.length) {
    ...
    q =
      keys.emails.length === 1
        ? q.eq('to_email', keys.emails[0])
        : q.in('to_email', keys.emails);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
    for (const row of data || []) {
      if (claimMatchesRecipient(row, keys)) found.set(row.id, row);
...
function formatClaimsMenu(claims, mode = 'outgoing') {
  if (!claims.length) {
    return mode === 'outgoing'
      ? 'No pending claims.\nSend to a phone, email, Telegram, or GitHub: flizy send 0.001 to 234… | to you@email.com | to @user on telegram'
