...
const QUALIFYING_KIND_SET = new Set(Object.values(QUALIFYING_KINDS));
 ...
 * Pure. Any confirmed on-chain Flizy receipt counts (claim, send, buy, sell,
 * swap, add/remove liquidity). Only a failed receipt is out.
 *
 * @param {{
 *   accountId?: string,
 *   kind?: string,
...
function isQualifyingFirstTx(p) {
  ...
  return QUALIFYING_KIND_SET.has(p.kind);
}
...
async function inviteStatsFor(supabase, inviterAccountId) {
