...
const { isUsernameReserved, reservedKey } = require('./username');
...
const INVITE_CODE_FORMAT = /^[0-9a-hjkmnp-tv-z]{10}$/;
const INVITE_ISSUE_TRIES = 8;
...
async function ensureInviteCode(supabase, accountId) {
  ...
  for (let i = 0; i < INVITE_ISSUE_TRIES; i += 1) {
    ...
    try {
      ...
      if (!isMissingRelation(err) && !isMissingRelation(err.cause)) {
        throw err;
      }
    }
 ...
 */
async function attributeSignup(supabase, { inviteeAccountId, code }) {
