...
async function assertEscrowSolvent(provider, opts = {}) {
  ...
  // After a new hold, liability increases by extraWei; balance should already include it.
  // For pre-payout: need balance >= liability (includes this claim) + gas for next ops.
  const needWei = liabilityWei + extraWei + gasBuffer;
  const ok = balanceWei >= needWei || balanceWei >= liabilityWei + extraWei;

  return {
    ok: balanceWei >= liabilityWei + extraWei,
    ...
    pendingCount: count,
    shortfallEth:
      balanceWei >= liabilityWei + extraWei
        ? '0'
        : ethers.formatEther(liabilityWei + extraWei - balanceWei),
  };
}
